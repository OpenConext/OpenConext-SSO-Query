...
public class DisableCacheInterceptor implements HandlerInterceptor {
    ...
    private static final String HTTP_10_PROTOCOL = "HTTP/1.0";

    private static final String NO_CACHE_HEADER_HTTP_10 = "no-cache";

    private static final String NO_CACHE_HEADER_HTTP_11 = "no-store, no-cache, must-revalidate";

    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) {
        if (HTTP_10_PROTOCOL.equalsIgnoreCase(request.getProtocol())) {
            response.setHeader(HttpHeaders.PRAGMA, NO_CACHE_HEADER_HTTP_10);
        } else {
            response.setHeader(HttpHeaders.CACHE_CONTROL, NO_CACHE_HEADER_HTTP_11);
        }
